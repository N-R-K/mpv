...
Currently only works on linux/unix
--]]
...
local function on_thumbnailer_request(_, req)
    msg.debug("received thumbnailer request: " .. utils.to_string(req))
    ...
    elseif req.hover_sec and req.x and req.y and req.w and req.h then
        ...
        state.current_request = NIL_REQUEST
        msg.error("user-data/osc/thumbnailer: received malformed property")
    end
end
...
-- TODO: windows support (currently only need mkdir + rm -rf)
